...
function editorSet() {
  const { editor, data } = this;
  if (data.settings.mode === 'read') return;
  if (data.rows._[data.selector.ri] &&
    data.rows._[data.selector.ri].cells[data.selector.ci] &&
    data.rows._[data.selector.ri].cells[data.selector.ci].cannotActive) return;
